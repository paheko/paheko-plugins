	...
	{foreach from=$list->iterate() item="product"}
		...
		{/if}
			<tr>
				...
				<td class="num">
					{if $product.stock < 0}{tag status="red" label=$product.stock}
					{elseif $product.stock <= 5}{tag status="orange" label=$product.stock}
					{elseif $product.stock <= 10}{tag status="greyblue" label=$product.stock}
					{else}{tag status="green" label=$product.stock}{/if}
				</td>
