...
.qty input, .total input[type=text] {
	width: 5em !important;
		...
		<fieldset>
			...
			<table class="list">
				...
				<tr data-denomination="{$value}">
					...
					<td class="total">{input type="text" readonly="readonly" name=""}</td>
				</tr>
				{/foreach}
				</tbody>
				<tfoot>
					<tr>
						...
						<td class="total">{input type="text" readonly="readonly" name="total_notes"}</td>
					</tr>
				</tfoot>
			</table>
		</fieldset>
			...
			<table class="list">
				...
				<tfoot>
					<tr>
						...
						<td class="total">{input type="text" readonly="readonly" name="total_coins"}</td>
					</tr>
				</tfoot>
			</table>
		</fieldset>
...
f.onsubmit = () => {
	...
	var total = parseFloat(f.total_notes.value || 0);
	total += parseFloat(f.total_coins.value.replace(',', '.') || 0);
	total = g.formatMoney(total*100);
...
function calculateTable(table) {
	...
	for (var i = 0; i < list.length; i++) {
		...
		row.querySelector('.total input').value = g.formatMoney(total);
		table_total += total;
	}

	table.querySelector('tfoot input').value = g.formatMoney(table_total);
}

$('fieldset table').forEach(t => {
	t.querySelectorAll('tbody tr .qty input').forEach(i => { i.onkeyup = () => calculateTable(t); });
});
{/literal}
</script>
