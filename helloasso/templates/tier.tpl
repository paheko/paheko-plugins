{include file="_head.tpl" title="%s — Configurer le tarif"|args:$tier.name}
...
<form method="post" action="{$self_url}">
	{if $f.type === 'Membership'}
		<fieldset>
			...
			<dl>
				{input type="select" name="id_fee" default_empty="— Ne pas inscrire —" label="Tarif auquel inscrire le membre" options=$fees source=$tier}
	...
	<fieldset>
		...
		<dl>
			{input type="list" target="!acc/charts/selector.php?types=6&key=code" name="account_code" label="Compte de recette" source=$account}
	...
	<fieldset>
		...
		<dl>
			{input type="radio-btn" name="create_user" value=1 source=$tier label="Chercher à lier un membre existant, sinon créer un nouveau membre" prefix_title="Liaison avec les membres" prefix_required=true required=true}
			...
			{input type="radio-btn" name="create_user" value=0 source=$tier label="Ne pas chercher à lier aux membres"}
		</dl>
	</fieldset>
