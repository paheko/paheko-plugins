...
<form method="post" action="{$self_url}">
	<fieldset>
		...
		<dl>
			...
			{if count($projects)}
				{input type="select" required=false default_empty="— Projet —" options=$projects source=$option name="id_project" label="Projet analytique" help="Laisser vide pour utiliser le projet défini pour la campagne."}
			{/if}
		</dl>
	</fieldset>
