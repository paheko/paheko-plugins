...
class Channel extends Entity
{
	...
	public function prune(): void
	{
		...
		if ($this->max_history) {
			$last_id = $db->firstColumn('SELECT id FROM plugin_chat_messages WHERE id_channel = ? ORDER BY id DESC LIMIT ?,1;', $this->id(), $this->max_history);

			if ($last_id) {
				$db->preparedQuery('DELETE FROM plugin_chat_messages WHERE id_channel = ? AND id_file IS NULL AND id < ?;', $this->id(), $last_id);

				$i = $em->iterate('SELECT * FROM @TABLE WHERE id_channel = ? AND id_file IS NOT NULL AND id < ?;', $this->id(), $last_id);
				foreach ($i as $row) {
					$row->delete();
				}
			}
	...
	public function sendMessage(User $user, string $text, string $type): Message
	{
		$now = time();
	...
	{
		return $this->sendMessage($user, $text, Message::TYPE_TEXT);
	}

	public function comment(User $user, string $text): Message
	{
		return $this->sendMessage($user, $text, Message::TYPE_COMMENT);
	}
